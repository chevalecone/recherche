...
void MRT_moment(int j, Lattice lat, double** M)
{
...
double** MRT_S(int Q, double nu, double cs, double dt) // Matrice S des temps de relaxation relatif aux différents moments
{
	double** matrix = new double*[Q];
	for ( int j=0;j<Q;j++)
	{
...
void MRT_collision( int j, double** f_star, double** C, Lattice lat,  int Q, double dt)
{
	...
	for (int k=0;k<Q;k++)
	{
		for ( int i=0;i<Q;i++)
		{
		sum[0]+=C[k][i]*(lat.m_[j][i]-lat.m0_[j][i]);
		}
		f_star[j][k] = lat.f_[j][k]-sum[0];
		sum[0]=0;
	}
...
void MRT_forcing_collision(int j, double** f_star, double** C, Lattice lat, double dt, double** F, double** C3, double** F_bar)
{
...
void MRT_forcing(int j, int Q, double cs, double* omega_i, double** xi, double* Fi, double** F_bar, Lattice lat)
{
	//Rappel : F_bar = omega_i * [ci*F/cs² + uF : (cici-cs²I)/cs^4]
	//Rappel : F = invM*(I- 1/2 * S)*M*F_bar

	double* sum = new double[1]; //produit scalaire ci * F
	double* sum2 = new double[1]; //somme de uF:(cici-cs²I)
	for (int k=0;k<Q;k++)
	{
		sum[0] = (xi[k][0]*Fi[0] + xi[k][1]*Fi[1]);
		sum2[0] = (lat.u_[j][0]*Fi[0]*(xi[k][0]*xi[k][0]-cs*cs) + lat.u_[j][0]*Fi[1]*xi[k][1]*xi[k][0] + lat.u_[j][1]*Fi[0]*xi[k][0]*xi[k][1] + lat.u_[j][1]*Fi[1]*(xi[k][1]*xi[k][1]-cs*cs));
		F_bar[j][k] = omega_i[k]*( 1/(cs*cs) * sum[0] + 1/(cs*cs*cs*cs) * sum2[0]);
	}
